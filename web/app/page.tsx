...
import { Stack, type PaneDef } from "@/components/Stack";
...
stdlib only: this machine's Application Control policy
blocks pandas' compiled extensions, and at n=271 there is
nothing pandas would buy us.
"""
...
export default function Index() {
  ...
  const panes: Record<string, PaneDef> = {
    /* ── the index ────────────────────────────────────────────────────── */
    index: {
      title: "index",
      node: (
    ...
    notebook: {
      ...
      node: (
        <>
          ...
          <pre className="code">{`33 of 33 published claims re-derived independently,
in pandas, from the raw source.
  ...
  piece 02 · exactly one cutoff reproduces …          1        1   True
  piece 02 · intervals clear of 1                    92       92   True
  piece 02 · sleep is at least as flat             True     True   True
  …`}</pre>

          <p className="path">
            analysis/notebooks/audit.ipynb · generated by make_audit.py
          </p>
          <p className="txt dim">
            GitHub renders notebooks, so the whole audit is readable in the browser without running
            anything. The generator executes each cell and embeds its real output, which is why the
            numbers above are output rather than prose. Two guards, different jobs:{" "}
